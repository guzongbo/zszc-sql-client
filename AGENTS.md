...
- 优先检查 Tauri command 与前端调用之间的字段命名和序列化是否保持 `snake_case` 一致
- 重点关注本地 SQLite、MySQL 连接信息、查询历史与草稿缓存是否存在敏感信息泄露风险
- 对 SQL 执行、结构对比和本地持久化相关改动，重点检查错误传播、空值处理和异常路径
- 若改动涉及 Rust 服务层或核心比对逻辑，优先指出可能导致数据误比对、误覆盖或结果不一致的问题
